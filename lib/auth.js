module.exports = {
    ...
    statusUi:function(request, response){
        ...
        if(this.isLogined(request, response)){
            ...
            if(request.session.is_admin){
                ...
                authStatusUi += `<a class="navbar-brand" href="/auth/cart" id="management">장바구니</a>`
            }
        }
        return authStatusUi;
    },
    ...
    productUi:function(request, response, productId){
        ...
        <input type="number" sid="count" class="form-control" name="count" max="100" min="1" value=1>
        <button type="submit" class="btn btn-success">상품 구매</button>
        </form>
        `;
